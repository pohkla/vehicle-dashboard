...
app = FastAPI(title="Vehicle Dashboard v4.2 Stable PDF Export")
...
function renderCharts(){destroy();const totalLine=filteredDays.map(d=>d.motorcycle+d.pickup+d.sedan);dailyChart=new Chart(box('dailyChart'),{type:'bar',data:{labels:filteredDays.map(d=>d.date.slice(0,5)),datasets:[{label:'🏍 รถจักรยานยนต์',data:filteredDays.map(d=>d.motorcycle),backgroundColor:'#2563eb',borderRadius:8,stack:'vehicle'},{label:'🚛 รถกระบะ',data:filteredDays.map(d=>d.pickup),backgroundColor:'#f97316',borderRadius:8,stack:'vehicle'},{label:'🚗 รถยนต์เก๋ง',data:filteredDays.map(d=>d.sedan),backgroundColor:'#16a34a',borderRadius:8,stack:'vehicle'},{type:'line',label:'📈 รวมทั้งหมด',data:totalLine,borderColor:'#111827',backgroundColor:'#111827',borderWidth:3,pointRadius:4,pointHoverRadius:6,tension:.35,yAxisID:'y'}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{font:{family:'Prompt',weight:'700'},usePointStyle:true,boxWidth:10}},tooltip:{bodyFont:{family:'Prompt'},titleFont:{family:'Prompt',weight:'700'}}},scales:{x:{stacked:true,grid:{color:'rgba(15,23,42,.06)'},ticks:{font:{family:'Prompt'}}},y:{stacked:true,beginAtZero:true,grid:{color:'rgba(15,23,42,.08)'},ticks:{precision:0,font:{family:'Prompt'}}}}}})}
...
function exportPDF(){
 ...
 const statusText=box('status')?box('status').textContent:'';
 const totalAmount=box('totalAmount')?box('totalAmount').textContent:'0';
 const motorCount=box('motorCount')?box('motorCount').textContent:'0';
 const pickupCount=box('pickupCount')?box('pickupCount').textContent:'0';
 const sedanCount=box('sedanCount')?box('sedanCount').textContent:'0';
 ...
 const html=[
   ...
   '<div class="meta">'+escapeHtml(statusText)+' • Export: '+escapeHtml(printedAt)+'</div>',
   '<div class="summary">',
   '<div class="card"><div class="label">ยอดรวมทั้งหมด</div><div class="value">'+escapeHtml(totalAmount)+'</div></div>',
   '<div class="card"><div class="label">รถจักรยานยนต์</div><div class="value">'+escapeHtml(motorCount)+'</div></div>',
   '<div class="card"><div class="label">รถกระบะ</div><div class="value">'+escapeHtml(pickupCount)+'</div></div>',
   '<div class="card"><div class="label">รถยนต์เก๋ง</div><div class="value">'+escapeHtml(sedanCount)+'</div></div>',
   '</div>',
   '<table>',
 ...
 if(!win){
   alert('Browser บล็อก popup กรุณาอนุญาต popup แล้วลอง Export PDF อีกครั้ง');
   return;
 }
