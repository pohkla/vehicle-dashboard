...
APP_VERSION = "v6.7 Company KPI Detail + Color Fix"
...
function renderCharts(){
 ...
 const datasets=[
   {label:'🔵 บริษัทกลางฯ RVP',data:companyData.map(d=>d.RVP),backgroundColor:'#2563eb',borderRadius:8,stack:'company'},
   {label:'🔴 ERGO',data:companyData.map(d=>d.ERGO),backgroundColor:'#dc2626',borderRadius:8,stack:'company'},
   {label:'🔵 ไทยไพบูลย์ TPB',data:companyData.map(d=>d.TPB),backgroundColor:'#0ea5e9',borderRadius:8,stack:'company'}
 ];
   ...
   options:{
     ...
     plugins:{
       ...
       tooltip:{
         ...
         callbacks:{afterBody:(items)=>{if(!items.length)return '';const i=items[0].dataIndex;const d=companyData[i];return ['รวม: '+d.total+' คัน','บริษัทกลางฯ RVP: '+d.RVP+' คัน','ERGO: '+d.ERGO+' คัน','ไทยไพบูลย์ TPB: '+d.TPB+' คัน'];}}
       }
     },
     scales:{
...
def dashboard_page() -> str:
    return DASHBOARD_HTML.replace("Vehicle Cumulative Dashboard", f"Vehicle Cumulative Dashboard • {APP_VERSION}")
