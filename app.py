...
app = FastAPI(title="Vehicle Dashboard v4.3 Sparkline Hover Animation")
...
<section class="kpi-grid">
 <div class="kpi"><div class="icon">🏍</div><div class="value" id="motorCount">0</div><div class="title">รถจักรยานยนต์</div><div class="spark-wrap" id="motorSpark"></div><div class="spark-caption"><span>Trend</span><span id="motorTrend">-</span></div></div>
 <div class="kpi"><div class="icon">🚛</div><div class="value" id="pickupCount">0</div><div class="title">รถกระบะ</div><div class="spark-wrap" id="pickupSpark"></div><div class="spark-caption"><span>Trend</span><span id="pickupTrend">-</span></div></div>
 <div class="kpi"><div class="icon">🚗</div><div class="value" id="sedanCount">0</div><div class="title">รถยนต์เก๋ง</div><div class="spark-wrap" id="sedanSpark"></div><div class="spark-caption"><span>Trend</span><span id="sedanTrend">-</span></div></div>
 <div class="kpi"><div class="icon">🚘</div><div class="value" id="allCount">0</div><div class="title">จำนวนรถรวมทั้งหมด</div><div class="spark-wrap" id="allSpark"></div><div class="spark-caption"><span>Trend</span><span id="allTrend">-</span></div></div>
</section>
<section class="section-grid"><div class="panel"><h2>จำนวนรถรายวัน + แนวโน้มรวม</h2><div class="chart-hover-note">Hover ที่แท่งกราฟเพื่อ Highlight วันนั้น และ Dim วันอื่น</div><div class="chart-wrap"><canvas id="dailyChart"></canvas></div></div><div class="hybrid-card"><div class="hybrid-head"><div><h2 style="margin:0">สัดส่วนประเภทรถ</h2><div class="hybrid-label">Hybrid Breakdown Card</div></div><div><div class="hybrid-total" id="hybridTotal">0</div><div class="hybrid-label">คันทั้งหมด</div></div></div><div class="breakdown-list" id="breakdownList"></div></div></section>
...
function exportPDF(){const rows=flattenRows(viewDays.length?viewDays:filteredDays);const printedAt=new Date().toLocaleString('th-TH');const html=['<!DOCTYPE html>','<html lang="th"><head><meta charset="UTF-8"><title>Vehicle Dashboard PDF</title>','<link href="https://fonts.googleapis.com/css2?family=Prompt:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">','<style>@page{size:A4 landscape;margin:12mm}body{font-family:Prompt,Arial,sans-serif;color:#172033}h1{font-size:22px}.meta{font-size:12px;color:#667085;margin-bottom:14px}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#2563eb;color:#fff;text-align:left;padding:7px}td{border-bottom:1px solid #e5e7eb;padding:6px;vertical-align:top}tr:nth-child(even) td{background:#f8fafc}</style></head><body>','<h1>Vehicle Cumulative Dashboard</h1><div class="meta">'+escapeHtml(box('status').textContent)+' • Export: '+escapeHtml(printedAt)+'</div>','<table><thead><tr><th>วันที่</th><th>ประเภทรถ</th><th>บริษัท</th><th>รายการ</th></tr></thead><tbody>',rows.map(r=>'<tr><td>'+escapeHtml(r.date)+'</td><td>'+escapeHtml(r.type)+'</td><td>'+escapeHtml(r.company)+'</td><td>'+escapeHtml(r.item)+'</td></tr>').join(''),'</tbody></table></body></html>'].join('');const win=window.open('', '_blank');if(!win){alert('Browser บล็อก popup กรุณาอนุญาต popup แล้วลอง Export PDF อีกครั้ง');return}win.document.open();win.document.write(html);win.document.close();win.focus();setTimeout(()=>win.print(),700)}
