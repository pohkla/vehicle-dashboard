...
APP_VERSION = "v6.12.3 Daily Money Detail"
...
function amountLine(net,collected){return `<div class="item-money"><span>ยอดสุทธิ ${money(net)} บาท</span><span>ยอดเก็บจริง ${money(collected)} บาท</span></div>`}
function buildDetails(day){const daySum=sumDay(day);return `<div class="day-money-summary"><div class="money-pill total"><span>ยอดเงินรวมประจำวัน</span><strong>${money(daySum.collected)} บาท</strong><small>ยอดเก็บจริง</small></div><div class="money-pill"><span>ยอดสุทธิตามระบบ</span><strong>${money(daySum.net)} บาท</strong><small>รวมทุกคัน</small></div></div>`+day.groups.map(g=>{const gSum=sumItems(g.items);return `<div class="vehicle-group"><div class="vehicle-title vehicle-title-row"><span>${g.icon} ${g.title} (${g.items.length} คัน)</span><span class="vehicle-money">สุทธิ ${money(gSum.net)} บาท • เก็บจริง ${money(gSum.collected)} บาท</span></div>${g.company?`<span class="company">${g.company}</span>`:''}<ul>${g.items.map(i=>`<li><div class="vehicle-item-row"><span>${itemText(i)}</span>${amountLine(itemNet(i),itemCollected(i))}</div></li>`).join('')}</ul></div>`}).join('')}
