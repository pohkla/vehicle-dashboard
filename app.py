...
app = FastAPI(title="Vehicle Dashboard v4.4 Export Summary Clean UI")
...
<section class="kpi-grid">
 <div class="kpi"><div class="icon">🏍</div><div class="value" id="motorCount">0</div><div class="title">รถจักรยานยนต์</div></div>
 <div class="kpi"><div class="icon">🚛</div><div class="value" id="pickupCount">0</div><div class="title">รถกระบะ</div></div>
 <div class="kpi"><div class="icon">🚗</div><div class="value" id="sedanCount">0</div><div class="title">รถยนต์เก๋ง</div></div>
 <div class="kpi"><div class="icon">🚘</div><div class="value" id="allCount">0</div><div class="title">จำนวนรถรวมทั้งหมด</div></div>
</section>
...
<script>
let report=null,allDays=[],filteredDays=[],viewDays=[],dailyChart=null;let currentPage=1,pageSize=8;const box=id=>document.getElementById(id);const money=n=>Math.round(n||0).toLocaleString('th-TH');function destroy(){if(dailyChart)dailyChart.destroy()}function setupRange(){const dates=allDays.map(d=>d.isoDate).filter(Boolean).sort();box('startDate').value=dates[0]||'';box('endDate').value=dates[dates.length-1]||''}function flattenRows(days){const rows=[];days.forEach(day=>day.groups.forEach(g=>g.items.forEach(item=>rows.push({date:day.date,type:g.title,company:g.company||'',item}))));return rows}
function animateNumber(el,target){const end=Number(target)||0;const start=Number((el.textContent||'0').replace(/,/g,''))||0;const duration=520;const t0=performance.now();function tick(now){const p=Math.min(1,(now-t0)/duration);const eased=1-Math.pow(1-p,3);el.textContent=money(start+(end-start)*eased);if(p<1)requestAnimationFrame(tick);else el.textContent=money(end)}requestAnimationFrame(tick)}
function createSparkline(data,color){if(!data||!data.length)return '';const max=Math.max(...data),min=Math.min(...data),range=max-min||1;const w=120,h=38,pad=3;const pts=data.map((v,i)=>{const x=data.length===1?w/2:pad+(i/(data.length-1))*(w-pad*2);const y=pad+(1-((v-min)/range))*(h-pad*2);return [x,y]});const line=pts.map(p=>p.join(',')).join(' ');const area=line+' '+pts[pts.length-1][0]+','+(h-pad)+' '+pts[0][0]+','+(h-pad);return `<svg class="sparkline" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><polygon class="spark-area" points="${area}" fill="${color}"></polygon><path d="M ${pts.map(p=>p.join(' ')).join(' L ')}" stroke="${color}"></path></svg>`}
function setTrend(id,data){const el=box(id);if(!el){return}if(!data||data.length<2){el.textContent='-';el.className='trend-flat';return}const first=data[0],last=data[data.length-1],diff=last-first;const pct=first?Math.round((diff/first)*100):diff?100:0;el.textContent=diff>0?`+${pct}% ↑`:diff<0?`${pct}% ↓`:'0% →';el.className=diff>0?'trend-up':diff<0?'trend-down':'trend-flat'}
function renderSparklines(){const motor=filteredDays.map(d=>d.motorcycle),pickup=filteredDays.map(d=>d.pickup),sedan=filteredDays.map(d=>d.sedan),all=filteredDays.map(d=>d.motorcycle+d.pickup+d.sedan);box('motorSpark').innerHTML=createSparkline(motor,'#2563eb');box('pickupSpark').innerHTML=createSparkline(pickup,'#f97316');box('sedanSpark').innerHTML=createSparkline(sedan,'#16a34a');box('allSpark').innerHTML=createSparkline(all,'#111827');setTrend('motorTrend',motor);setTrend('pickupTrend',pickup);setTrend('sedanTrend',sedan);setTrend('allTrend',all)}
function makeColors(data,color,hoverIndex){return data.map((_,i)=>hoverIndex===null||hoverIndex===undefined?color:(i===hoverIndex?color:color.replace('1)', '.18)').replace('#2563eb','rgba(37,99,235,1)').replace('#f97316','rgba(249,115,22,1)').replace('#16a34a','rgba(22,163,74,1)').replace('#111827','rgba(17,24,39,1)')))}
function colorWithAlpha(hex,alpha){const map={'#2563eb':'37,99,235','#f97316':'249,115,22','#16a34a':'22,163,74','#111827':'17,24,39'};return `rgba(${map[hex]||'37,99,235'},${alpha})`}
function applyChartHighlight(index){if(!dailyChart)return;const colors=['#2563eb','#f97316','#16a34a'];dailyChart.data.datasets.forEach((ds,di)=>{if(ds.type==='line'){ds.borderColor=index==null?'#111827':colorWithAlpha('#111827',.95);ds.backgroundColor=ds.borderColor;ds.pointBackgroundColor=ds.data.map((_,i)=>index==null||i===index?'#111827':colorWithAlpha('#111827',.18));return}ds.backgroundColor=ds.data.map((_,i)=>index==null||i===index?colors[di]:colorWithAlpha(colors[di],.18))});dailyChart.update('none')}
...
function exportExcel(){
 ...
 const summaryRows=[
   {หมวด:'ยอดเงินรวมทั้งหมด',ยอด:box('totalAmount').textContent,หน่วย:'บาท'},
   {หมวด:'รถยนต์',ยอด:box('carAmount').textContent.replace(' บาท',''),หน่วย:'บาท'},
   {หมวด:'รถจักรยานยนต์',ยอด:box('motorAmount').textContent.replace(' บาท',''),หน่วย:'บาท'},
   {หมวด:'รถจักรยานยนต์',ยอด:box('motorCount').textContent,หน่วย:'คัน'},
   {หมวด:'รถกระบะ',ยอด:box('pickupCount').textContent,หน่วย:'คัน'},
   {หมวด:'รถยนต์เก๋ง',ยอด:box('sedanCount').textContent,หน่วย:'คัน'},
   {หมวด:'จำนวนรถรวมทั้งหมด',ยอด:box('allCount').textContent,หน่วย:'คัน'}
 ];
 const wsSummary=XLSX.utils.json_to_sheet(summaryRows);
 const wsDetail=XLSX.utils.json_to_sheet(rows.map(r=>({วันที่:r.date,ประเภทรถ:r.type,บริษัท:r.company,รายการ:r.item})));
...
function exportPDF(){
 ...
 const totalAmount=box('totalAmount').textContent;
 const carAmount=box('carAmount').textContent;
 const motorAmount=box('motorAmount').textContent;
 const motorCount=box('motorCount').textContent;
 const pickupCount=box('pickupCount').textContent;
 const sedanCount=box('sedanCount').textContent;
 const allCount=box('allCount').textContent;
 const html=[
 '<!DOCTYPE html>',
 '<html lang="th"><head><meta charset="UTF-8"><title>Vehicle Dashboard PDF</title>',
 '<link href="https://fonts.googleapis.com/css2?family=Prompt:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">',
 '<style>@page{size:A4 landscape;margin:12mm}body{font-family:Prompt,Arial,sans-serif;color:#172033}h1{font-size:22px;margin:0 0 6px}.meta{font-size:12px;color:#667085;margin-bottom:14px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px}.card{border:1px solid #e5e7eb;border-radius:12px;padding:10px;background:#f8fafc}.label{font-size:11px;color:#667085}.value{font-size:19px;font-weight:800;color:#2563eb}.sub{font-size:11px;color:#667085;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#2563eb;color:#fff;text-align:left;padding:7px}td{border-bottom:1px solid #e5e7eb;padding:6px;vertical-align:top}tr:nth-child(even) td{background:#f8fafc}.money-table{margin-bottom:14px}.money-table th{background:#111827}.money-table td{font-size:11px}</style></head><body>',
 '<h1>Vehicle Cumulative Dashboard</h1>',
 '<div class="meta">'+escapeHtml(box('status').textContent)+' • Export: '+escapeHtml(printedAt)+'</div>',
 '<div class="summary">',
 '<div class="card"><div class="label">ยอดเงินรวมทั้งหมด</div><div class="value">'+escapeHtml(totalAmount)+'</div><div class="sub">บาท</div></div>',
 '<div class="card"><div class="label">รถจักรยานยนต์</div><div class="value">'+escapeHtml(motorCount)+'</div><div class="sub">คัน</div></div>',
 '<div class="card"><div class="label">รถกระบะ</div><div class="value">'+escapeHtml(pickupCount)+'</div><div class="sub">คัน</div></div>',
 '<div class="card"><div class="label">รถยนต์เก๋ง</div><div class="value">'+escapeHtml(sedanCount)+'</div><div class="sub">คัน</div></div>',
 '</div>',
 '<table class="money-table"><thead><tr><th>หมวดยอดเงิน</th><th>ยอด</th></tr></thead><tbody>',
 '<tr><td>รถยนต์</td><td>'+escapeHtml(carAmount)+'</td></tr>',
 '<tr><td>รถจักรยานยนต์</td><td>'+escapeHtml(motorAmount)+'</td></tr>',
 '<tr><td>รวมทั้งหมด</td><td>'+escapeHtml(totalAmount)+' บาท</td></tr>',
 '</tbody></table>',
 '<table><thead><tr><th>วันที่</th><th>ประเภทรถ</th><th>บริษัท</th><th>รายการ</th></tr></thead><tbody>',
