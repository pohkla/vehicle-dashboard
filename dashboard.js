...
function toggleCarSummary(){const slide=box('carSummarySlide');const row=box('carSummaryToggle');if(!slide)return;slide.classList.toggle('open');const isOpen=slide.classList.contains('open');row?.classList.toggle('open',isOpen);const label=row?.querySelector('.toggle-label');if(label)label.textContent=isOpen?'ซ่อนรายละเอียด':'ดูแยกประเภท'}
function render(selected='all'){
 ...
 if(box('expenseTotalAmount'))box('expenseTotalAmount').textContent=money(expense.total)+' บาท';
 if(box('profitTotalAmount'))box('profitTotalAmount').textContent=money(profit.total)+' บาท';
 if(box('carExpenseAmount'))box('carExpenseAmount').textContent=money(expense.car)+' บาท';
 if(box('carProfitAmount'))box('carProfitAmount').textContent=money(profit.car)+' บาท';
 if(box('pickupExpenseAmount'))box('pickupExpenseAmount').textContent=money(expense.pickup)+' บาท';
 if(box('pickupProfitAmount'))box('pickupProfitAmount').textContent=money(profit.pickup)+' บาท';
 if(box('sedanExpenseAmount'))box('sedanExpenseAmount').textContent=money(expense.sedan)+' บาท';
 if(box('sedanProfitAmount'))box('sedanProfitAmount').textContent=money(profit.sedan)+' บาท';
 if(box('motorExpenseAmount'))box('motorExpenseAmount').textContent=money(expense.motorcycle)+' บาท';
 if(box('motorProfitAmount'))box('motorProfitAmount').textContent=money(profit.motorcycle)+' บาท';
...
function sumItems(items){return (items||[]).reduce((acc,item)=>{acc.net+=itemNet(item);acc.collected+=itemCollected(item);acc.expense+=itemExpense(item);return acc},{net:0,collected:0,expense:0})}
function sumDay(day){return (day.groups||[]).reduce((acc,g)=>{const s=sumItems(g.items);acc.net+=s.net;acc.collected+=s.collected;acc.expense+=s.expense;return acc},{net:0,collected:0,expense:0})}
function amountLine(net,collected,expense){return `<div class="item-money"><span class="net">ยอดสุทธิ ${money(net)} บาท</span><span class="collected">ยอดเก็บจริง ${money(collected)} บาท</span>${expense?`<span class="expense-col">ค่าใช้จ่าย ${money(expense)} บาท</span>`:''}</div>`}
function buildDetails(day){const daySum=sumDay(day);return `<div class="day-money-summary"><div class="money-pill net"><span>ยอดสุทธิตามระบบ</span><strong>${money(daySum.net)} บาท</strong><small>รวมทุกคัน</small></div><div class="money-pill collected"><span>ยอดเงินรวมประจำวัน</span><strong>${money(daySum.collected)} บาท</strong><small>ยอดเก็บจริง</small></div><div class="money-pill"><span>ค่าใช้จ่ายประจำวัน</span><strong>${money(daySum.expense)} บาท</strong><small>จาก Excel</small></div></div>`+day.groups.map(g=>{const gSum=sumItems(g.items);return `<div class="vehicle-group"><div class="vehicle-title vehicle-title-row"><span>${g.icon} ${g.title} (${g.items.length} คัน)</span><span class="vehicle-money"><span class="money-chip net">สุทธิ ${money(gSum.net)} บาท</span><span class="money-chip collected">เก็บจริง ${money(gSum.collected)} บาท</span>${gSum.expense?`<span class="money-chip">จ่าย ${money(gSum.expense)} บาท</span>`:''}</span></div>${g.company?`<span class="company">${g.company}</span>`:''}<ul>${g.items.map(i=>`<li><div class="vehicle-item-row"><span>${itemText(i)}</span>${amountLine(itemNet(i),itemCollected(i),itemExpense(i))}</div></li>`).join('')}</ul></div>`}).join('')}
...
function exportExcel(){
 ...
 const summaryRows=[
  ...
  {หมวด:'ค่าใช้จ่ายรวมทั้งหมด',ยอด:safeText('expenseTotalAmount'),หน่วย:'บาท'},
  {หมวด:'คงเหลือสุทธิรวมทั้งหมด',ยอด:safeText('profitTotalAmount'),หน่วย:'บาท'},
 ...
 const wsDetail=XLSX.utils.json_to_sheet(rows.map(r=>({วันที่:r.date,ประเภทรถ:r.type,บริษัท:r.company,รายการ:r.item,ยอดสุทธิตามระบบ:r.net_amount,ยอดเก็บจริง:r.collected_amount,ค่าใช้จ่าย:r.expense_amount,คงเหลือสุทธิ:r.profit_amount})));
